...
class MultiCropTransform:
    ...
    def __call__(self, x):
        return torch.cat([T(x).unsqueeze(0) for T in self.transforms])
...
class SBI_Dataset(Dataset):
	...
	def pretrain_collate_fn(self,batch):
		img_f,img_r=zip(*batch)
		data={}
		# print("size of img ",img_f[0].shape)
		data['positives']= torch.cat([self.multicroptransform(torch.from_numpy(img)) for img in img_r])#torch.cat([torch.tensor(img_r).float(),torch.tensor(img_f).float()],0)
		data['negatives']= torch.cat([self.multicroptransform(torch.from_numpy(img)) for img in img_f])
		data['img'] = torch.cat((data['positives'],data['negatives']))
		data['label']=torch.tensor([0]*len(data['positives'])+[1]*len(data['negatives']))
		# print("Size of image ",data['img'].shape)
		return data
...
if __name__=='__main__':
	...
	dataloader = torch.utils.data.DataLoader(image_dataset,
					...
					shuffle=True,
					collate_fn=image_dataset.pretrain_collate_fn,
					num_workers=0,
